(function($) {
    ...
    sevengits.DeactivateFeedbackForm = function(plugin) {
        var self = this;
        var strings = sgdoa_deactivate_feedback_form_strings;
        this.plugin = plugin;

        // Dialog HTML
        var element = $('\
			<div class="sgits-oad-deactivate-dialog" data-remodal-id="' + plugin.slug + '">\
				<form>\
					...
					<ul class="sgits-oad-deactivate-reasons"></ul>\
					...
					<p class="sgits-oad-deactivate-dialog-buttons">\
        ...
        // Reasons list
        var ul = $(element).find("ul.sgits-oad-deactivate-reasons");
    ...
    $(document).ready(function() {
        ...
        for (var i = 0; i < sgdoa_deactivate_feedback_form_plugins.length; i++) {
            var plugin = sgdoa_deactivate_feedback_form_plugins[i];
