...
const OptionForm = ( { formMethods, index, option } ) => {
	...
	const quantityLimited =
		...
		( optionType === 'product' && [ 'checkbox', 'image_buttons' ].includes( productDisplayStyle ) );
				...
				<tbody>
					...
					{ supportsImages() && (
						...
						>
							...
							/>
							{ [ 'checkbox', 'radio', 'dropdown' ].includes( optionType ) && (
								<Controller
									control={ formMethods.control }
									...
									render={ ( { field } ) => {
										return (
											<CheckboxControl
												isClassicStyle
												label={ sprintf(
													/* translators: %s: Input type (either `checkbox` or `radio button`) */
													__(
					...
					{ optionType === 'file_upload' && settings?.use_live_preview && (
						<OptionFormRow
							name={ 'live_preview' }
								...
								<>
									...
									<Notice isDismissible={ false } status="info">
										...
										<a
											...
											class="wpo-live-preview-documentation"
										>
											<Dashicon icon="book" />
