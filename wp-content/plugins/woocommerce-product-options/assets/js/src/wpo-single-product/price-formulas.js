...
export const processFormulaVariable = ( variable, form ) => {
	...
	switch ( variable.type ) {
		...
		case 'text_labels_option':
			...
			choiceElements.forEach( ( element, index ) => {
				...
				choiceVariable.choices[ `choice${index}` ] = {
					...
					value: isChecked ? evaluateFieldValue( parseFloat( element.dataset.formulaValue ) ) : 0,
				};
			} );
		...
		case 'price_formula_option':
			return { [ variableName ]: {
				value: evaluateFieldValue( parseFloat( inputField.dataset.optionPrice ) )
			} };
...
export const evaluateFormula = ( field ) => {
	...
	const formulaParams = {
		...
		...lCaseFunctionNames( getLogicalFunctions() ),
		...lCaseFunctionNames( getCustomFunctions() ),
	};
